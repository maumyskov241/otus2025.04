...
const onSubmit = function (values){
  ...
  emit('create-product', values);

  window.setTimeout(() => (show.value = false),1000);
}
...
<template>
    ...
    <div class="alert alert-success" v-show="show">
      <strong>Осторожно!</strong> Товар добавлен.
    </div>
    </div>
    <div class="p-3 border bg-light" style="margin-top:0;">
        ...
        <div class="col-12">
          <Field name="description" :rules="{ required: true, min: 20, max: 255 }" v-slot="{ field, meta }">
            ...
            <textarea v-bind="field" id="description" class="form-control" v-model.trim="description" placeholder="Описание"
              maxlength="255" :class="meta.touched ? (meta.valid ? 'valid' : 'invalid') : ''" />
          </Field>
